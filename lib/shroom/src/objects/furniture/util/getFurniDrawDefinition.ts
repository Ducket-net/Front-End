...
export function getFurniDrawDefinition(
  ...
  furniSize = furniSize ?? 64;

  //check window uses
  //@ts-ignore
  if (window["_tempFurniSizeUses"] == 0) {
    //@ts-ignore
    furniSize = window["_tempFurniSize"];
  }

  //@ts-ignore
  if (window["_tempFurniSizeUses"] == 1) {
    //@ts-ignore
    furniSize = window["_tempFurniSize"];
    //@ts-ignore
    window["_tempFurniSize"] = undefined;
  }
  ...
  const getAssetName = (char: string, frame: number) =>
    `${type}_${furniSize}_${char}_${direction}_${frame}`;
  ...
  const mask = assetsData.getAsset(`${type}_${furniSize}_${direction}_mask`);
